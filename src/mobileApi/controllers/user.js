...
module.exports = {
  userRegister: async (req, res) => {
    try {
      ...
      const isUserMobileExist = await User.findOne({ mobile: req.body.mobile.trim()});
      if (isUserMobileExist) return sendResponse(constant.MESSAGE.MOBILE_ALREADY_EXIST,res, constant.CODE.EMAILEXIST, {}, 0);

      const isUserEmailExist = await User.findOne({email: req.body.email.trim().toLowerCase() });
      if (isUserEmailExist) return sendResponse(constant.MESSAGE.EMAIL_ALREADY_EXIST,res, constant.CODE.EMAILEXIST, {}, 0);
      ...
      return sendResponse(constant.MESSAGE.REGISTERED,res, constant.CODE.SUCCESS, { user:userSaved }, 1);
    } catch (error) {
  ...
  userLogin: async (req, res) => {
    try {
      ...
      if (!user) return sendResponse(constant.MESSAGE.INVALID_USER, res, constant.CODE.NOT_FOUND, {}, 0);
      ...
      return sendResponse(constant.MESSAGE.LOGIN, res, constant.CODE.SUCCESS, {user, token: token }, 1);
    } catch (error) {
  ...
  changePassword: async (req, res) => {
    try {
      ...
      if (!isUserExist) return sendResponse(constant.MESSAGE.EMAIL_NOT_REGISTERED, res, constant.CODE.NOT_EXIST, {}, 0);
  ...
  forgetPassword:async(req,res)=>{
    try {
     if(!req.body.mobile) return sendResponse("Mobile Number is Mandatory",res,constant.CODE.INPUT_VALIDATION,{},0)
      const mobile=req.body.mobile.trim();
      const user=await User.findOne({mobile:req.body.mobile});
      if(!user) return sendResponse(constant.MESSAGE.USER_NOT_FOUND,res,constant.CODE.NOT_FOUND,{},0);

      const otp=helper.generateOtp()
      const savedOtp=await OTP.findOneAndUpdate({mobile:req.body.mobile},{mobile,otp},{upsert:true,new: true});
      return sendResponse("Otp Send successfully",res,constant.CODE.SUCCESS,{savedOtp},1)
    } catch (error) {
      console.log(error)
  ...
  resetPassword:async(req,res)=>{
    try {
      ...
      if(!mobile) return sendResponse("Mobile Number is Mandatory",res,constant.CODE.INPUT_VALIDATION,{},0);
    ...
    if (!user) return sendResponse(constant.MESSAGE.USER_NOT_FOUND, res, constant.CODE.NOT_FOUND, {}, 0);
      ...
      return sendResponse("Password updated successfully",res,constant.CODE.SUCCESS,{},1)
    } catch (error) {
      return sendResponse("Internal Server Error", res, constant.CODE.INTERNAL_SERVER_ERROR, {}, 0);
  ...
  profile: async (req, res) => {
    try {
      ...
      return sendResponse("Profile data fetched successfully", res, constant.CODE.SUCCESS, { userProfile }, 1);
    } catch (error) {
      return sendResponse("Internal Server Error", res, constant.CODE.INTERNAL_SERVER_ERROR, {}, 0);
  ...
  updateProfile:async(req,res)=>{
    try {
      ...
      if (!user) return sendResponse(constant.MESSAGE.USER_NOT_FOUND, res, constant.CODE.NOT_FOUND, {}, 0);
      return sendResponse("Profile updated successfully", res, constant.CODE.SUCCESS, { user }, 1);
    } catch (error) {
      return sendResponse("Internal Server Error", res, constant.CODE.INTERNAL_SERVER_ERROR, {}, 0);
    }
  }
